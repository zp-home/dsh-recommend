      ...
      const verification = await fetch('/dsh-recommend/verification.json', { cache: 'no-store' })
        .then(async (response) => response.ok ? await response.json() as {
          plugins?: Record<string, {
            staticSecurity?: { status: 'passed' | 'warnings' | 'incomplete'; risk: 'low' | 'medium' | 'high'; commit: string; checkedAt: string; findings?: Array<{ rule: string; risk: 'low' | 'medium' | 'high'; file: string; line: number; message: string; family?: string; confidence?: string; disposition?: string; basis?: string; remediation?: string; baselineRisk?: 'low' | 'medium' | 'high'; protections?: string; downgrade?: string }> } | null
