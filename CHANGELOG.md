...
- **市场 GitHub 安全队列**：每 15 分钟选择最多 6 个缺失或过期插件并发执行源码静态扫描，回执绑定仓库/commit/规则版本，汇总到 `data/verification.json`；不执行目标插件代码、不安装依赖、不影响排行分数或安装资格。
- **排行榜验证标签**：设置页展示市场静态扫描、发布者 `clean` 标准环境回执和显式本机兼容检测；未检测状态保持可安装且明确可见。
- **dsh-dev-sandbox 联动**：市场只接受用户手动准备的本地目录，通过现有隔离镜像以 `clean` 或 `host-web` 检测；不下载、不构建、不安装远端插件。

### 变更

- **全量分数徽章**：`scripts/badge.mjs` 现在为所有上榜项目生成分数徽章，并清理已不在当前排行榜中的旧分数文件；精选认证徽章不受影响。
...
- **同步频率调整**：数据全量同步调整为每 8 小时一次（UTC cron `17 */8 * * *`），降低 GitHub API、深扫与 Actions 资源消耗；仍保留手动触发。
